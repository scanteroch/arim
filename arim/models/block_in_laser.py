"""
Model for solid block for laser induced phased arrays (LIPAs)

Everything is based on the model for solid block for the probe in contact.
...
- the model is not reciprocal (swapping the transmitter and the receiver gives
  different results)
- surface waves need to be modelled outside these functions

Limits of the forward model:

- the scaling between the L and the T directivity of the elements is dubious
    * An additional relationship between the two has been added to address
      this issue.
...
def tx_ray_weights(
    ...
    if use_directivity:
        if path.modes[0] is c.Mode.L:
            directivity_func = model.directivity_linesource_laser_tx_l
        elif path.modes[0] is c.Mode.T:
            directivity_func = model.directivity_linesource_laser_tx_t
        else:
            raise RuntimeError
...
def rx_ray_weights(
    ...
    if use_directivity:
        if path.modes[0] is c.Mode.L:
            directivity_func = model.directivity_2d_rectangular_laser_l
        elif path.modes[0] is c.Mode.T:
            directivity_func = model.directivity_2d_rectangular_laser_t
        else:
            raise RuntimeError
        weights_dict["directivity"] = directivity_func(
            ray_geometry.conventional_out_angle(0),
    ...
    ###########################################################################
    # MOD BY SERGIO CANTERO CHINCHILLA
    # From Matt explanation: I believe there’s a factor required in the 
    # model_coefficients from the reception path proportional to the wave 
    # velocity of the mode which interacts with the array 
    # (i.e. view L-TL would require *vL; L-TT would require a factor *vT).
    block = path.materials[0]
    if path.modes[-1] is c.Mode.L:
        reception_coeff = block.longitudinal_vel
    elif path.modes[-1] is c.Mode.T:
        reception_coeff = block.transverse_vel
    else:
        raise RuntimeError
    weights *= scat_normalisation * reception_coeff
    # weights *= scat_normalisation
...
def ray_weights_for_wall(
    ...
    if use_directivity:
        if path.modes[0] is c.Mode.L:
            directivity_func_tx = model.directivity_linesource_laser_tx_l
        elif path.modes[0] is c.Mode.T:
            directivity_func_tx = model.directivity_linesource_laser_tx_t
        if path.modes[-1] is c.Mode.L:
            directivity_func_rx = model.directivity_2d_rectangular_laser_l
        elif path.modes[-1] is c.Mode.T:
            directivity_func_rx = model.directivity_2d_rectangular_laser_t
